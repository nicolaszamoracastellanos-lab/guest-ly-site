...
14. **iPad: phone only for 1.0** (`ios.supportsTablet` stays `false`; Part 9 wave, Sep 18 2026). Reasons: (a) adding iPad to a shipped app can never be undone in a later update, while phone only keeps the choice open; (b) iPadOS 26 runs iPad apps in freely resizable windows and `UIRequiresFullScreen` is deprecated, so a universal app must support all four orientations and any size, and this app is `orientation: "portrait"` with photo-led full-bleed phone layouts; (c) the approved design canvas (Direction A, 54 artboards) has no tablet artboards; (d) the product is a phone companion: invitation, RSVP, concierge, door check-in with the camera; (e) a universal app needs a 13 inch screenshot set per language, phone only needs the 6.9 inch set; (f) App Review still runs iPhone apps on iPad in a compatibility window, so that window was audited on the iPad mini and iPad Pro 13 simulators (the mini window is the 375x667 layout, the Pro window a tall phone) and must stay clean. Nicolas can overrule this before the production build; after release the opposite decision is not reversible. Going universal later is a small change: flip the flag, drop the portrait lock for iPad, add 13 inch screenshots, rebuild.
15. **Large screen rule** (Part 9 wave). Android has no phone-only lock, and with target SDK 36 Android 16 ignores the portrait lock on windows of 600 dp and wider. So every surface sits in one centered column (`COLUMN`, `MAX_CONTENT_WIDTH` 560 in `src/ui/tokens.ts`, applied by `Screen` and by every list through the exported helper), the tab bar is at most 520 wide, sheets at most 640, and full-bleed photos are a clipped `View` plus an `Image` with `FILL`, never `ImageBackground` with a local `require`. Two layout facts learned the hard way: `FILL` uses percent sizes because a local image keeps its pixel size under `absoluteFill`; and percent sizes resolve against the parent's CONTENT box, so inside a padded parent the photo goes in a `COVER` view (insets only). Checked on the web rig at 360, 390, 430, 768, 1024 and 1440.
16. **Language ladder** (Part 9 wave, D-041): the language chosen in the app, then the device language when it is English or Spanish, then the guest's own language from the guest record, then the wedding's `locale_default`, then English. The last three only ever apply on a phone set to a third language. Before, the wedding default overrode the device language, so an English phone could flip to Spanish after opening a wedding. The ladder is in the header of `src/i18n/index.tsx`.
17. **Dynamic Type policy** (Part 9 wave): text follows the system size up to 1.3 (`src/ui/Text.tsx`), inputs included. Two documented exceptions: tab bar labels do not scale (10 pt under an icon, five across 375 pt), and stat tile labels do not scale while the tile number scales to 1.1 (three tiles share a 327 pt row and words broke mid word). At the largest setting the More menu goes to one column on phones narrower than 380 pt.
