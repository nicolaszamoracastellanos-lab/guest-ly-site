...
import { Screen, TopBar, Wordmark, IconButton, BigTitle, Card, ListRow, Badge, EmptyState, Skeleton, Stack, SectionLabel } from "@/ui";
      ...
      <Stack gap={10} style={{ marginTop: 20 }}>
        ...
        {tasks.length ? (
          <>
            ...
            <Card kind="solid" padding={2} style={{ paddingHorizontal: 18 }}>
              {tasks.map((t, i) => (
                <ListRow key={t.id} title={t.title} sub={t.detail || null} below={<Badge label={copy.planner.taskStatus[t.status]} kind={t.status === "in_progress" ? "gold" : "mute"} />} onPress={() => router.push("/planner/tasks")} last={i === tasks.length - 1} />
              ))}
            </Card>
          </>
        ) : null}
